...
`terraform-provider-openwrt`.  Substitute
...
1. Create the schema by running the following command:
    ...
    ```text
    warning: resource openwrt_something not found in provider map; skipping
    warning: resource openwrt_something_else not found in provider map; skipping
    warning: data source openwrt_something not found in provider map; skipping
    warning: data source openwrt_something_else not found in provider map; skipping
    ```
...
1. **Add resource mappings:** For each resource in the provider, add an entry in
   the `Resources` property of the `tfbridge.ProviderInfo`, e.g.:
    ...
    "openwrt_something":      {Tok: tfbridge.MakeResource(mainPkg, mainMod, "Something")},
    "openwrt_something_else": {Tok: tfbridge.MakeResource(mainPkg, mainMod, "SomethingElse")},
    ```
...
1. **Add data source mappings:** For each data source in the provider, add an entry in the `DataSources` property of the `tfbridge.ProviderInfo`, e.g.:
    ...
    "openwrt_something":      {Tok: tfbridge.MakeDataSource(mainPkg, mainMod, "getSomething")},
    "openwrt_something_else": {Tok: tfbridge.MakeDataSource(mainPkg, mainMod, "getSomethingElse")},
    ```
...
1. **Add provider configuration overrides (not typically needed):** Pulumi's
   ...
   `openwrt_API_KEY`.  Examples of common
   uses cases:
    ...
    "apikey": {
        Default: &tfbridge.DefaultInfo{
            EnvVars: []string{"openwrt_API_KEY"},
        },
    ```
...
1. Create a new Pulumi program in the `examples/` directory, e.g.:
    ...
    npm install
    yarn link @deposition-cloud/openwrt
    ```
